...
#include "yyl_log.h"
#include "ffmpeg4_1_3/ffmpeg.h"
#include "ffmpeg4_1_3/ffprobe.h"
...
static void ffmpeg_log_callback_yyl(void *ptr, int level, const char *fmt, va_list vl) {
    if (level <= AV_LOG_INFO && YYL_LOGCAT) {
        yyl_callback_log(ptr, level, fmt, vl);
    }
}

int yyl_main_ffmpeg(int argc, char **argv) {
    return yyl_main(argc, argv);
}
...
void setLogCallback() {
    av_log_set_callback(ffmpeg_log_callback_yyl);
}