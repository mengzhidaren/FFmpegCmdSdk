...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    mins %= 60;
    hours_sign = (pts < 0) ? "-" : "";
    int64_t total_secs = FFABS(pts) / 1000;//1000000//生成文件的总时长 毫秒
    onCallbackProgress(frame_number, total_secs);
...
int yyl_main(int argc, char **argv)
{
    LOGI("ffmpeg 命令开始");
    yyl_on_destroy();
    int i, ret;
